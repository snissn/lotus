...
accounting of the true size of the actor in the state datastore.`,
	...
	Action: func(cctx *cli.Context) error {
		...
		tsExec := filcns.NewTipSetExecutor()
		...
		worker := func(ctx context.Context, id int, wg *sync.WaitGroup, jobs <-chan address.Address, results chan<- actorStats) error {
			completed := 0
			...
			for {
				...
				case addr, ok := <-jobs:
					...
					actor, err := sm.LoadActor(workersCtx, addr, ts)
					if err != nil {
						return err
					}

					dag, err := newCacheNodeGetter(merkledag.NewDAGService(blockservice.New(bs, offline.Exchange(bs))), 8096)
					if err != nil {
						return err
					}

					actStats, err := collectStats(workersCtx, addr, actor, dag)
					if err != nil {
						return err
					}
		...
		for w := 0; w < numWorkers; w++ {
			wg.Add(1)
			go func(id int) {
				if err := worker(workersCtx, id, &wg, jobs, results); err != nil {
					select {
					case errChan <- err:
...
func collectStats(ctx context.Context, addr address.Address, actor *types.Actor, dag format.NodeGetter) (actorStats, error) {
	...
	oif, err := vm.DumpActorState(filcns.NewTipSetExecutor().NewActorRegistry(), actor, nd.RawData())
	if err != nil {
		return actorStats{}, err
	}
